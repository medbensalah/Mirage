...
namespace Mirage {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			MRG_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				MRG_CORE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			MRG_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					MRG_CORE_ERROR(module.GetErrorMessage());
					MRG_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			MRG_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		MRG_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		MRG_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		MRG_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		MRG_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			MRG_CORE_TRACE("  {0}", resource.name);
			MRG_CORE_TRACE("    Size = {0}", bufferSize);
			MRG_CORE_TRACE("    Binding = {0}", binding);
			MRG_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
