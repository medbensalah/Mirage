...
void Sandbox2D::OnAttach()
{
    ...
    m_particleProps.ColorBegin = {1.0f, 0.0f, 0.0f, 1.0f};
    m_particleProps.ColorEnd = {0.0f, 1.0f, 0.0f, 1.0f};
    ...
    Mirage::FramebufferSpecs specs;
    specs.Width = 1280;
    specs.Height = 720;
    m_Framebuffer = Mirage::Framebuffer::Create(specs);
}
...
void Sandbox2D::OnUpdate(float DeltaTime)
{
...

    {
        ...
        for(float y = -5.0f; y < 5.0f; y += 0.04f)
        {
            for(float x = -5.0f; x < 5.0f; x += 0.04f)
            {
    ...
    if(dockingEnabled)
    {
        ...
        if (ImGui::BeginMenuBar())
        {
            if (ImGui::BeginMenu("File"))
            {
                ...
                ImGui::Separator(); 
                if (ImGui::MenuItem("Exit")) { Mirage::Application::Get().Close(); }    
                ImGui::EndMenu();
            }
